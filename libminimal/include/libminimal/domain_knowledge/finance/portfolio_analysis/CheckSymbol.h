...
public:
    ...
    std::string CheckSym(){
        ...
        bool check= true;
        while(check){
            std::cout << "Enter name of ticker (e.g 'AAPL'): ";
            std::cin >> input;
            for (unsigned i = 0; i < SymList.size(); ++i){
                if (input == SymList.at(i)){
                    std::cout << std::endl << input << " found!" << std::endl;
                    return "./data/" + input + "_data.csv";
                }
            }
            std::cout << std::endl << input << " not found!" << std::endl;
        }
        //cout << "***RETURN***";
